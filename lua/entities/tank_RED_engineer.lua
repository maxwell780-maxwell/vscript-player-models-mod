...
ENT.Type = "anim"
ENT.PrintName = "RED Tank (engineer)"
ENT.Author = "Code Copilot & sourcegraph & maxwell"
ENT.Category = "maxwells custom tanks"
ENT.IconOverride = "vgui/icons/tank_engineer_icon"
ENT.Spawnable = true
...
function ENT:SpawnGibs()
    ...
    if IsValid(ragdoll) then
        ragdoll:SetModel("models/player/engineer.mdl")
        ...
        local painFlexID = ragdoll:GetFlexIDByName("painBig")
        if painFlexID then
            ragdoll:SetFlexWeight(painFlexID, 1.0) -- Set to maximum
        else
            -- Try alternative flex names if painBig doesn't exist
            local alternativeFlexes = {"painBig", "painBig", "painBig", "painBig"}
