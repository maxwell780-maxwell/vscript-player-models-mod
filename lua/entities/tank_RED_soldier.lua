...
function ENT:SpawnGibs()
    ...
    if IsValid(ragdoll) then
        ragdoll:SetModel("models/player/soldier.mdl")
        ...
        -- Find the painBig flex ID
        local painFlexID = ragdoll:GetFlexIDByName("painSmall")
        if painFlexID then
            ragdoll:SetFlexWeight(painFlexID, 1.0) -- Set to maximum
        else
            -- Try alternative flex names if painBig doesn't exist
            local alternativeFlexes = {"painSmall", "painSmall", "painSmall", "painSmall"}
