hook.Add("PlayerSpawn", "JuliusVIPHealthBonus", function(ply)
    -- Wait a tick to ensure the player model is set
    timer.Simple(0.1, function()
        if IsValid(ply) and ply:GetModel() == "models/vip/player/julius/julius.mdl" then
            ply:SetHealth(175)
...
hook.Add("PlayerModelChanged", "JuliusVIPHealthBonusModelChange", function(ply)
    if IsValid(ply) and ply:GetModel() == "models/vip/player/julius/julius.mdl" then
        ply:SetHealth(175)
        ply:SetMaxHealth(175)
    else
